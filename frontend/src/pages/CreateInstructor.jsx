...
const CreateInstructor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:3000/app/v1/head-dept/create-instructor", formData, {
      ...
      if (response.data.success) {
        ...
        setErrors({ submit: response.data.message || 'Failed to create instructor' });
      }
    } catch (error) {
      console.error('Error creating instructor:', error);
      const errorMessage = error.response?.data?.message || 'Failed to create instructor. Please try again.';
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">Create New Instructor</h1>
            <p className="text-white/70">Add a new instructor to your department</p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-8 shadow-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-8">
              <button
                ...
                className="flex-1 bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 disabled:from-gray-500 disabled:to-gray-600 text-white py-3 px-6 rounded-xl font-medium transition-all duration-200 hover:scale-105 hover:shadow-lg disabled:hover:scale-100 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
                  <>
